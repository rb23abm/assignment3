...
def worldbank_data(filename):
    ...
    Returns:
    - years_df (DataFrame): Transposed DataFrame with 'Country Name'
      as the index.
...
years_df, countries_df = \
    worldbank_data('API_FI.RES.TOTL.CD_DS2_en_csv_v2_6298254.csv')
data = countries_df[
    ['Country Name', 'Indicator Name'] + list(map(str, range(2000, 2021)))]
data = data.dropna()
print(data.head())

growth = data[["Country Name", "2010", "2020"]].copy()
growth = growth.assign(
    Growth_Percentage=\
        lambda x: 100.0 * \
            (pd.to_numeric(x["2020"], errors='coerce') - \
             pd.to_numeric(x["2010"], errors='coerce')) / \
                pd.to_numeric(x["2010"], errors='coerce'))
print(growth.head())
...
sns.scatterplot(
    x=growth["2020"], y=growth["Growth_Percentage"],
    label="Total reserves (includes gold, current US$)", color="green")
...
plt.scatter(
    growth["2020"], growth["Growth_Percentage"],
    10, labels, marker="o", cmap="ocean")
...
uk_data = years_df[
    ['United Kingdom']].loc['2000':'2020'].reset_index().rename(
        columns={'index': 'Years', 'United Kingdom': 'Reserves (USD)'})
uk_data.columns.name = 'Index'
uk_data['Years'] = \
    uk_data['Years'].apply(lambda x: pd.to_numeric(x, errors='coerce'))
uk_data['Reserves (USD)'] = uk_data['Reserves (USD)'].apply(
...
param, covar = opt.curve_fit(
    exponential, uk_data["Years"], uk_data["Reserves (USD)"], p0=(1.1e5, 0.2))
uk_data["fit"] = exponential(uk_data["Years"], *param)
plt.figure(figsize=(8, 6))
sns.lineplot(
    data=uk_data, x="Years", y="Reserves (USD)",
    label="Reserves (USD)", color="red")
sns.lineplot(
    data=uk_data, x="Years", y="fit", label="Exponential Fit", color="green")
plt.xlabel("Year")
...
sns.lineplot(
    ...
    label="Reserves (USD)", color="red")
sns.lineplot(
    x=years_future, y=predictions,
    label="Best Fitting Function", color='blue')
sns.lineplot(
    x=years_future, y=predictions - confidence_range,
    color='green', alpha=0.2, label="Confidence Range")
sns.lineplot(
    x=years_future, y=predictions + confidence_range,
